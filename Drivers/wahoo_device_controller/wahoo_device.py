...
# Wahoo Device Interface
"""
WARNING: This code was directly copied from `kickr_climb_and_smart_trainer/wahoo_device.py` and then modified - so some unexpected artifacts may be present.

This class should act as an interface between the game communication and the individual Wahoo devices.
It should:
* Establish communication with the bike.
* Initialise the device controllers.
* Establish communication with the game.
* Instigate reading device data.
* Instigate writing device commands.
* Listen for game publications.
* Publish device data.
* Modularly structured:
    0 The communication methods should be segregated from their applications.
    0 Reason: Basically we want to be able to switch from MQTT easily - so if a controller returns data from the bike to the interface, then the interface will call a method to push that data to the game or mobile app. 
    Code using MQTT, CoAP or any other communication method we select to use should only appear in this final method during this process or similiar processes. 
"""
